...
export default function ListOrder({ item }) {
    ...
    return (
        <>
            <p className="w-min"><FontAwesomeIcon onClick={() => setShow(!show)} className={show ? 'rotate-0 sm:text-3xl text-sm duration-200' : 'rotate-180 sm:text-3xl text-sm duration-200'} icon={faAngleUp} /></p>
            <p className="overflow-x-auto">{item._id}</p>
            <p>{item.total}</p>
